...
- default (analysis-only): stream each video through the optical-flow
  motion tracker and the GOP chunker; no store is written. Fast,
...
from cctv_zarr.archive import CloudArchive, get_storage_client
from cctv_zarr.capture import VideoCapture, iter_frames, read_info
from cctv_zarr.chunker import build_chunk_records
from cctv_zarr.config import Settings
from cctv_zarr.exceptions import ArchiveError, VideoOpenError
...
CSV_COLUMNS = [
    ...
    "first_event_offset_s", "longest_event_s", "peak_magnitude",
    "process_fps", "store_mb", "query_chunks_fetched",
    "query_frames", "error",
]
...
def analyse_video(
    ...
    """Analysis-only pass: motion events + chunk records, no store.
    ...
    tracker = MotionTracker(settings.flow, fps=info.fps)
    flow_results = []
    frame_cap = min(
    ...
    with VideoCapture(video_path) as cap:
        for index, frame in iter_frames(
            ...
            flow_results.append(tracker.update(frame, index))
    elapsed = time.perf_counter() - started
    if not flow_results:
        raise VideoOpenError(video_path)
    tracker.finish()
    events = tracker.events()
    flags = movement_flags(events, len(flow_results))
    start_time = datetime.fromtimestamp(
        video_path.stat().st_mtime, tz=timezone.utc,
    )
    records = build_chunk_records(
        settings.gop, flags, start_time, info.fps,
    )
    movement_chunks = sum(r.movement for r in records)
    longest = max(
        (
            (e.end_frame - e.start_frame + 1) / info.fps
            for e in events
        ),
        default=None,
    )
    return {
        ...
        "first_event_offset_s": (
            round(events[0].start_frame / info.fps, 2)
            if events else ""
        ),
        "longest_event_s": (
            round(longest, 2) if longest is not None else ""
        ),
        "peak_magnitude": round(
            max(r.magnitude for r in flow_results), 4,
        ),
    ...
    for i, (video_path, label) in enumerate(videos, 1):
        ...
        try:
            row.update(analyse_video(
                settings, video_path, args.max_frames,
            ))
            if args.full:
